 ...
 *
 * @source
 * https://github.com/skortchmark9/riMap/blob/master/deprecated/ZoomPane.java
 */
...
public class ZoomPane extends JPanel implements MouseWheelListener {
    ...
    /**
     * @author samkortchmar Mostly taken from various StackOverflow posts such
     * as:
     * http://stackoverflow.com/questions/13155382/jscrollpane-zoom-relative-to-mouse-position
     * But I've modded it (clumsily) to support the pan-by-dragging and
     * zoom-by-scrolling behavior we want for the map. There are problems with
     * the panning right now, I think it's redrawing too quickly which makes it
     * jerky.
     *
     * Supports: Zoom in/out to mouse pointer from mousewheel Click and drag to
     * pan (not quite right) Zoom in/out via SHIFT-EQUALS or MINUS
     */
    ...
    public ZoomPane(String filePath) {
    ...
    ZoomPane() {
        try {
            background = ImageIO.read(new File("./resources/Choose.png"));
            scaled = background;
            scaledSize = new Dimension(background.getWidth(this), background.getHeight(this));
        } catch (IOException ex) {
        }
    ...
    public void setZoom(float value) {
        ...
        if (zoom != value && value > 0 && mouse != null) {
            ...
            scaled = background.getScaledInstance(width, height, Image.SCALE_DEFAULT);
    ...
    protected void centerInViewport() {
        ...
        if (container instanceof JViewport port) {
